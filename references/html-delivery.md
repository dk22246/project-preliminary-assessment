...
- Word 使用 `scripts/render_report_word.py`，保持同一七部分目录和数据字段；股权图以 SVG 插入，不使用浮动文本框。
...
- `scripts/run_report_pipeline.py` 必须提供已通过 `scripts/validate_equity_evidence.py` 的 `--equity-evidence` 和 `--node`，并会自动运行 `scripts/verify_html_layout.mjs`。门禁检查整页横向宽度、全部报告表格的滚动/文本/单元格重叠、SVG 容器与元素边界，以及 SVG 文本互相重叠；失败时禁止生成 PDF 或 Word。PDF和Word需再做文本一致性及页面渲染检查。
